...
Adaptive execution pipelines (`direct`, `focused`, `coordinated`) are active by default. Work boundaries and risk escalate based on runtime tool effects and observable workspace mutations. Adaptive execution starts with a 30-tool-call slice; at 80% consumption, the active model enters a tool-free checkpoint mode and may request another slice, clamped to 50 calls. Checkpoints are append-only durable records and user-visible transcript events. Plan revisions remain immutable, may add newly discovered work, and completed items may reopen with evidence. Slice extensions stop after six grants, and repeated calls, targets, errors, or consecutive slices without verified progress stop new work and enter a separate tool-free final-response phase. This phase does not consume work-step, work-tool, or execution model-call allowance, and it reports completed work plus technical, verification, or procedural harness blockers. Explicit `execution_budget` values remain hard ledger ceilings and are never raised by adaptive policy.
...
- If final-step completion is blocked by an open taskflow, workflow, or child session, AtomCLI records and projects a visible `budget_exhausted/step_limit` outcome instead of creating a continuation that cannot use tools or ending with a reasoning-only message.
...
- Independent review steps in the taskflow are runtime-owned; the agent cannot mark them completed without actual reviewer invocation evidence and a matching PASS verdict.
