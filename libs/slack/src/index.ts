...
import { createAtomcli, type ToolPart } from "@atomcli/sdk"
...
console.log("🚀 Starting atomcli server...")
const atomcli = await createAtomcli({
  port: 0,
})
console.log("✅ Atomcli server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await atomcli.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new atomcli session...")
    const { client, server } = atomcli
    ...
    console.log("✅ Created atomcli session:", createResult.data.id)
  ...
  console.log("📝 Sending to atomcli:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Atomcli response:", JSON.stringify(result, null, 2))
