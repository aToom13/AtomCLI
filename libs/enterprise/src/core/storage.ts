...
export namespace Storage {
  ...
  function s3(): Adapter {
    const bucket = process.env.ATOMCLI_STORAGE_BUCKET!
    const region = process.env.ATOMCLI_STORAGE_REGION || "us-east-1"
    const client = new AwsClient({
      region,
      accessKeyId: process.env.ATOMCLI_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.ATOMCLI_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://s3.${region}.amazonaws.com`, bucket)
  }

  function r2() {
    const accountId = process.env.ATOMCLI_STORAGE_ACCOUNT_ID!
    const client = new AwsClient({
      accessKeyId: process.env.ATOMCLI_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.ATOMCLI_STORAGE_SECRET_ACCESS_KEY!,
    })
