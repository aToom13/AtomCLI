import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@atomcli/sdk/v2"
import { SessionTurn } from "@atomcli/ui/session-turn"
import { SessionReview } from "@atomcli/ui/session-review"
import { DataProvider } from "@atomcli/ui/context"
import { DiffComponentProvider } from "@atomcli/ui/context/diff"
import { CodeComponentProvider } from "@atomcli/ui/context/code"
import { WorkerPoolProvider } from "@atomcli/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createEffect, createMemo, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@atomcli/ui/logo"
import { IconButton } from "@atomcli/ui/icon-button"
import { ProviderIcon } from "@atomcli/ui/provider-icon"
import { createDefaultOptions } from "@atomcli/ui/pierre"
import { iife } from "@atomcli/util/iife"
...
import { Diff as SSRDiff } from "@atomcli/ui/diff-ssr"
import { clientOnly } from "@solidjs/start"
import { type IconName } from "@atomcli/ui/icons/provider"
import { Meta, Title } from "@solidjs/meta"
import { Base64 } from "js-base64"

const ClientOnlyDiff = clientOnly(() => import("@atomcli/ui/diff").then((m) => ({ default: m.Diff })))
const ClientOnlyCode = clientOnly(() => import("@atomcli/ui/code").then((m) => ({ default: m.Code })))
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@atomcli/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/atomcli-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | AtomCLI</Title>
              </Show>
              <Meta name="description" content="atomcli - The AI coding agent built for the terminal." />
                          ...
                          <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                            <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                              ...
                              <div class="flex gap-3 items-center">
                                <IconButton
                                  as={"a"}
                                  href="https://github.com/anomalyco/atomcli"
                                  target="_blank"
                                  icon="github"
                                  variant="ghost"
                                />
