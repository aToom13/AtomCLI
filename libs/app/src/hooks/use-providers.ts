...
export const popularProviders = ["atomcli", "anthropic", "github-copilot", "openai", "google", "openrouter", "vercel"]
...
export function useProviders() {
  ...
  const paid = createMemo(() =>
    connected().filter((p) => p.id !== "atomcli" || Object.values(p.models).find((m) => m.cost?.input)),
  )
