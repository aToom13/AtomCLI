...
import { $ } from "bun"
import { createAtomcli } from "@atomcli/sdk"
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/aToom13/AtomCLI/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/atomcli packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/atomcli/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/atomcli/")) areas.add("core")
...
async function summarizeCommit(atomcli: Awaited<ReturnType<typeof createAtomcli>>, message: string): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await atomcli.client.session.create()
  const result = await atomcli.client.session
    .prompt({
      path: { id: session.data!.id },
      body: {
        model: { providerID: "atomcli", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], atomcli: Awaited<ReturnType<typeof createAtomcli>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(atomcli, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/aToom13/AtomCLI/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const atomcli = await createAtomcli({ port: 5044 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, atomcli)
    notes.push(...lines)
