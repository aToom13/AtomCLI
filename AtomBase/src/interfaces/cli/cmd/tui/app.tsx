...
function App() {
  ...
  // Extracted hooks — command palette, SDK events, clipboard
  ...
  createEffect(() => {
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`AtomCLI | ${title}`)
    }
  })
  ...
  onMount(() => {
    batch(() => {
      ...
      if (args.model) {
        ...
        local.model.set({ providerID, modelID }, { recent: true, manual: true })
      }
      if (args.sessionID) {
        route.navigate({
          type: "session",
...
function ErrorComponent(props: {
  ...
  const issueURL = new URL("https://github.com/aToom13/AtomCLI/issues/new")
