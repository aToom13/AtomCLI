...
export function useSDKEventHandlers() {
  ...
  sdk.event.on(TuiEvent.SubAgentActive.type, (evt) => {
    const { sessionId, agentType, description, parentSessionId, parentStepId, runtime, startedAt } = evt.properties
  ...
  sdk.event.on(TuiEvent.SubAgentActivity.type, (evt) => {
    const { sessionId, ...activity } = evt.properties
    subAgentCtx.recordActivity(sessionId, activity)
  })

  sdk.event.on(TuiEvent.SubAgentDone.type, (evt) => {
    const { sessionId, lastOutput, completedAt } = evt.properties
    subAgentCtx.markWaiting(sessionId, lastOutput, completedAt)
  })
  ...
  sdk.event.on(TuiEvent.SubAgentReactivate.type, (evt) => {
    const { sessionId, description, parentSessionId, parentStepId, startedAt } = evt.properties
    subAgentCtx.reactivate(sessionId, description, parentSessionId, parentStepId, startedAt)
  })
