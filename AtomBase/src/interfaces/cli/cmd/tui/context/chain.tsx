...
import { type AgentChain, Chain, type StepTodo, type SubStep } from "@/integrations/agent/chain"
...
export interface ChainContextValue {
  ...
  addStep: (
    ...
    description: string,
    todos?: StepTodo[],
    extra?: { stepId?: string; sessionId?: string; agentType?: string; dependsOn?: string[]; sessionID?: string },
  ) => void
...
export function ChainProvider(props: ParentProps) {
  ...
  const updateStepStatus = (status: string, tool?: string, sessionID?: string) => {
    updateChainForSession(key(sessionID), (current) => Chain.updateStepStatus(current, status as any, tool))
  }
  ...
  return <ChainContext.Provider value={value}>{props.children}</ChainContext.Provider>
}
