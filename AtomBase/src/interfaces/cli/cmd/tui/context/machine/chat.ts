  ...
  actions: {
    ...
    updateLifecycleState: assign({
      sessionStates: ({ context, event }) => {
        const { type, payload } = event
        const sessionID =
          payload.properties?.sessionID || payload.properties?.info?.sessionID || payload.properties?.part?.messageID // Using part's messageID somewhat as fallback if sessionID is omitted
