...
export function CodePanel(props: { width: number }) {
  ...
  const shouldShow = createMemo(() => fileTree.state.codePanelVisible && fileTree.hasOpenFiles())
  ...
  return (
    <Show when={shouldShow()}>
      ...
      >
        {/* Tab Bar */}
        <box flexDirection="row" backgroundColor={theme.backgroundMenu} flexShrink={0}>
          <For each={fileTree.state.openFiles}>
            {(file) => <FileTab file={file} active={file.path === fileTree.state.activeFile} />}
          </For>
        ...
        <Show when={activeFile()}>
          <box
            ...
            flexShrink={0}
          >
            <text fg={theme.textMuted}>{activeFile()!.language}</text>
