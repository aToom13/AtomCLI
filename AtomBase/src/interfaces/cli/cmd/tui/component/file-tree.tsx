        ...
        return (
          <box>
              ...
              {(focused: () => boolean) => (
                ...
                >
                  <Show when={entry.isDir}>
                    <text fg={theme.textMuted}>{fileTree.isDirExpanded(entry.path) ? "▼" : "▶"}</text>
                  </Show>
                  <text fg={theme.text}>
            ...
            <Show when={entry.isDir && fileTree.isDirExpanded(entry.path)}>
              <FileTreeNode dirPath={entry.path} depth={props.depth + 1} refreshKey={props.refreshKey} />
            </Show>
          </box>
        )
      }}
        ...
        return (
          ...
          >
            {(focused: () => boolean) => (
              ...
              >
                <text fg={theme.text}>
                  {getFileIcon(entry.name, entry.isDir)} <span style={{ fg: theme.textMuted }}>{relativePath()}</span>
                  {entry.name}
                </text>
              </box>
            )}
      ...
      <Show when={props.expanded}>
        ...
        <scrollbox flexGrow={1} paddingTop={1}>
          <Show
            ...
            fallback={
              /* Normal tree view */
              <FileTreeNode dirPath={directory()} depth={0} refreshKey={refreshKey()} />
            }
          >
            {/* Search results */}
            <SearchResults
