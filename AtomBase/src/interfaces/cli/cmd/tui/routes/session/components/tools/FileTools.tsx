...
export function Find(props: ToolProps<typeof FindTool>) {
  ...
  return (
    <InlineTool
      icon={isTree() ? "→" : "✱"}
      pending={isTree() ? "Listing directory..." : "Finding files..."}
      complete={isTree() ? props.input.path !== undefined : props.input.pattern}
      part={props.part}
    >
      <Show when={isTree()}>Find {props.input.path ? normalizePath(props.input.path) : ""}</Show>
      <Show when={!isTree()}>
