...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run atomcli",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for atomcli session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found atomcli session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`atomcli import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting atomcli...")
        UI.println()

        // Launch atomcli TUI with session ID if available
        const { spawn } = await import("child_process")
        const atomcliArgs = sessionId ? ["-s", sessionId] : []
        const atomcliProcess = spawn("atomcli", atomcliArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          atomcliProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`atomcli exited with code ${code}`))
          })
          atomcliProcess.on("error", reject)
        })
      },
    })
  },
