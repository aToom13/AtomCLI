...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run atomcli with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: AtomcliClient, sessionID: string) => {
    ...
    if (args.attach) {
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.ATOMCLI_AUTO_SHARE || args.share)) {
