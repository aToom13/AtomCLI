...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall atomcli and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall AtomCLI")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g atomcli-ai",
      pnpm: "pnpm uninstall -g atomcli-ai",
      bun: "bun remove -g atomcli-ai",
      yarn: "yarn global remove atomcli-ai",
      brew: "brew uninstall atomcli",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using AtomCLI!")
}
