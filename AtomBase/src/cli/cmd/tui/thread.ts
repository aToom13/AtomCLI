...
declare global {
  const ATOMCLI_WORKER_PATH: string
}
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start atomcli tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start atomcli in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    const workerPath = await iife(async () => {
      if (typeof ATOMCLI_WORKER_PATH !== "undefined") return ATOMCLI_WORKER_PATH
