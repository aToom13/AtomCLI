...
export async function rerank(
  ...
  limit: number = 5,
  routeModel?: ModelPurpose.ModelReference,
  sessionID?: string,
  ...
  try {
    const execution = await ModelPurpose.resolve("analysis", query, routeModel, sessionID)
    const language = execution.language
    const generateText = await getGenerateText()

    // Build prompt with numbered candidates
    const candidateList = candidates.map((c, i) => `[${i}] ${c.content.substring(0, 200)}`).join("\n")
...
Return ONLY the JSON array, no explanation.`
    ...
    const executionAttempt = sessionID
      ? await ExecutionRuntime.admitModelCall({
          ...
          estimateMicrousd: ExecutionRuntime.estimateMicrousd(execution.model, prompt, execution.model.limit.output),
        })
      : undefined
    let result
    try {
      ...
      if (result) executionAttempt?.settle(ExecutionRuntime.usageCostUsd(execution.model, result.usage))
      else executionAttempt?.uncertain()
    } catch (error) {
