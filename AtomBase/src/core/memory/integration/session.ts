...
export class SessionMemoryIntegration {
  ...
  static async learnFromMessage(message: string, model?: { providerID: string; modelID: string }): Promise<void> {
    try {
      ...
      const extracted = await SemanticLearningService.extractUserInformation(
        message,
        {
      ...
      if (extracted.preferences) {
        for (const pref of extracted.preferences) {
          await this.preferences.learn(pref.category, pref.key, pref.value, message.slice(0, 100))
   ...
   */
  static async processConversationTurn(
    userMessage: string,
    ...
    try {
      ...
      await this.learnFromMessage(userMessage, model)

      // Learn from assistant response
      await this.learnFromResponse(assistantResponse, userMessage, model)
  ...
  static async learnCodeStyle(code: string, language: string): Promise<void> {
    try {
      ...
      if (["javascript", "typescript", "jsx", "tsx"].includes(language)) {
        ...
        const withoutSemicolon = lines.filter(
          (l) => !l.trim().endsWith(";") && !l.trim().endsWith("{") && !l.trim().endsWith("}"),
        ).length
