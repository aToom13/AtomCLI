...
import type { MemoryItem, MemoryType, SearchOptions, MemoryStats, SearchResult } from "../types"
...
export class HybridStorage implements MemoryStorage {
  ...
  async deleteMany(ids: string[]): Promise<void> {
    await Promise.all([this.jsonStorage.deleteMany(ids), this.vectorStorage.delete(ids)])
  }
  ...
  async search(query: string, options?: SearchOptions): Promise<MemoryItem[]> {
    ...
    if (candidates.length <= 3 || options?.skipRerank) return candidates.slice(0, limit)
    ...
    try {
      ...
      return reranked.map((r) => itemMap.get(r.id)).filter((item): item is MemoryItem => item !== undefined)
    } catch {
  ...
  async clear(): Promise<void> {
    await Promise.all([this.jsonStorage.clear(), this.vectorStorage.clear()])
  }
