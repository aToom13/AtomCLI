...
export namespace SessionProcessor {
  ...
  const MAX_DEFERRED_TEXT_CHARS = 512_000
  const MAX_DEFERRED_TEXT_PARTS = 1_024
  const MAX_DEFERRED_PAYLOAD_BYTES = 2 * 1024 * 1024
  const log = Log.create({ service: "session.processor" })

  async function recordModelEvidence(
    model: Provider.Model,
    capabilities: ModelVerification.Capability[],
    error?: unknown,
    variant?: string,
    ...
    try {
      ...
      if (!provider) return
      await ModelVerification.observe(model, provider, capabilities, error, { variant, effectiveParams })
    } catch (recordError) {
      log.warn("failed to persist model verification evidence", {
      ...
      async process(
        ...
        while (true) {
          ...
          try {
            ...
            const observedCapabilities = new Set<ModelVerification.Capability>()

            // Direct stream — errors happen during fullStream iteration, caught below
            const stream = await LLM.stream(streamInput)
            const executionAttempt = stream.__executionAttempt
            verificationParams = stream.__verificationParams
            let executionSettled = false
            let executionCost = 0
            let executionUsageObserved = false
                ...
                switch (value.type) {
                    ...
                    if (match && match.state.status === "running") {
                      ...
                      await Session.updatePart({
                        ...
                        state: {
                          ...
                          input: normalizeToolInput(value.input),
                          error: repairedError,
                          metadata: { repair: RepairPlanner.classify(value.error, match.tool), applied },
                          time: {
                            start: match.state.time.start,
                            end: Date.now(),
                          },
                  ...
                  case "finish-step":
                    ...
                    await Session.updateMessage(
                      options?.deferText && terminalFinish
                    ...
                    if (
                      (await SessionCompaction.isOverflow({ tokens: usage.tokens, model: streamInput.model })) &&
                      !(options?.deferText && terminalFinish)
                    ) {
                  ...
                  case "text-delta":
                    if (currentText) {
                      ...
                      if (options?.deferText && deferredTextChars > MAX_DEFERRED_TEXT_CHARS) {
                        throw new Error(`Deferred final response exceeds ${MAX_DEFERRED_TEXT_CHARS} characters`)
                      }
                      if (value.providerMetadata) currentText.metadata = value.providerMetadata
                      if (currentText.text && !options?.deferText)
                        await Session.updatePart({
                          part: currentText,
                          delta: value.text,
                        })
                  ...
                  case "text-end":
                    if (currentText) {
                      ...
                      const beforeTransformLength = currentText.text.length
                      ...
                      deferredTextChars += currentText.text.length - beforeTransformLength
                      if (options?.deferText && deferredTextChars > MAX_DEFERRED_TEXT_CHARS) {
                        throw new Error(`Deferred final response exceeds ${MAX_DEFERRED_TEXT_CHARS} characters`)
                      }
                      if (currentText.text.trim()) observedCapabilities.add("text")
                      currentText.time = {
                        start: Date.now(),
                        end: Date.now(),
                      }
                      ...
                      if (options?.deferText) {
                        if (deferredTextParts.length + 1 > MAX_DEFERRED_TEXT_PARTS) {
                          throw new Error(`Deferred final response exceeds ${MAX_DEFERRED_TEXT_PARTS} text parts`)
                        }
                        deferredPayloadBytes += new TextEncoder().encode(JSON.stringify(currentText)).byteLength
                        if (deferredPayloadBytes > MAX_DEFERRED_PAYLOAD_BYTES) {
                          throw new Error(`Deferred final response exceeds ${MAX_DEFERRED_PAYLOAD_BYTES} bytes`)
                        }
                        deferredTextParts.push(currentText)
                      } else await Session.updatePart(currentText)
            ...
            if (executionUsageObserved) {
              executionAttempt?.settle(executionCost)
            ...
            if (observedCapabilities.size > 0) {
              await recordModelEvidence(
                streamInput.model,
                Array.from(observedCapabilities),
                undefined,
                streamInput.user.variant,
                verificationParams,
              )
            } else {
              await recordModelEvidence(
                streamInput.model,
                [],
                new Error("Model completed with empty output"),
                streamInput.user.variant,
                verificationParams,
              )
            }
            ...
            if (!input.abort.aborted && !(e instanceof ExecutionRuntime.BudgetExceededError)) {
                    ...
                    if (config.fallback?.enabled === false) {
                      ...
                      for (const fallbackModelID of fallbackModels) {
                        try {
                          ...
                          const routePolicy = Provider.routePolicy(streamInput.model)
                          const eligible = await Promise.all(
                            routePolicy.requiredCapabilities.map((capability) =>
                              Provider.isRouteEligible(fallbackModel, routePolicy, capability),
                            ),
                          )
                          if (eligible.some((value) => !value)) continue
                          ...
                          currentFallbackModel = routedFallback
          ...
          const deferTerminalFinish =
            options?.deferText &&
            (Boolean(input.assistantMessage.error) ||
              (input.assistantMessage.finish && !["tool-calls", "unknown"].includes(input.assistantMessage.finish)))
          if (!deferTerminalFinish) input.assistantMessage.time.completed = Date.now()
          const { completed: _completed, ...pendingTime } = input.assistantMessage.time
          await Session.updateMessage(
            deferTerminalFinish
              ? {
                  ...input.assistantMessage,
