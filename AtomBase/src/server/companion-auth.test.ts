...
describe("CompanionAuth", () => {
  ...
  beforeEach(() => {
    ...
    const { publicKey, privateKey } = globalThis.crypto.subtle.generateKey({ name: "Ed25519" }, true, [
      "sign",
      "verify",
  ...
  test("migrates a protocol-v2 device name to a stable installation id", () => {
    const deviceName = `legacy_${crypto.randomUUID()}`
    const deviceId = `installation_${crypto.randomUUID()}`
    CompanionAuth.registerDevice(deviceName, "dummy")

    expect(CompanionAuth.bindDeviceId(deviceName, deviceId)).toBe(true)
    expect(CompanionAuth.getDevice(deviceId)).toMatchObject({ deviceId, deviceName })

    CompanionAuth.removeDevice(deviceId)
  })
