...
import { SessionRevert } from "@/core/session/revert"
...
export const SessionToolRoute = new Hono()
  ...
  .post(
    ...
    validator(
      "json",
      z.lazy(() => SessionPrompt.CommandInput.omit({ sessionID: true })),
    ),
    async (c) => {
      const sessionID = c.req.valid("param").sessionID
      const body = c.req.valid("json")
  ...
  .post(
    ...
    validator(
      "json",
      z.lazy(() => SessionPrompt.ShellInput.omit({ sessionID: true })),
    ),
    async (c) => {
      const sessionID = c.req.valid("param").sessionID
      const body = c.req.valid("json")
  ...
  .post(
    ...
    validator(
      "json",
      z.lazy(() => SessionRevert.RevertInput.omit({ sessionID: true })),
    ),
    async (c) => {
      const sessionID = c.req.valid("param").sessionID
      const body = c.req.valid("json")
