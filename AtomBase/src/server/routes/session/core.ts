...
export const SessionCoreRoute = new Hono()
  ...
  .post(
    ...
    validator(
      "json",
      z.lazy(() => Session.create.schema.optional()),
    ),
    async (c) => {
  ...
  .delete(
    ...
    async (c) => {
      ...
      // Run the deferred memory retrospective while messages still exist;
      // Session.remove wipes history underneath it.
      await MemoryLifecycle.flush(sessionID)
  ...
  .post(
    ...
    validator(
      "json",
      z.lazy(() => Session.initialize.schema.omit({ sessionID: true })),
    ),
    async (c) => {
      const sessionID = c.req.valid("param").sessionID
      const body = c.req.valid("json")
  ...
  .post(
    ...
    validator(
      "json",
      z.lazy(() => Session.fork.schema.omit({ sessionID: true })),
    ),
    async (c) => {
      const sessionID = c.req.valid("param").sessionID
      const body = c.req.valid("json")
