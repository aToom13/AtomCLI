...
export const ESLint: Info = {
  ...
  async spawn(root) {
    ...
    if (!(await Bun.file(serverPath).exists())) {
      ...
      await fs.rename(extractedPath, finalPath).catch(async () => {
        // Windows: rename may fail on cross-device or locked files, fallback to copy+delete
        await fs.cp(extractedPath, finalPath, { recursive: true }).catch(() => {})
