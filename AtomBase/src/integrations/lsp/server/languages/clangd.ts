...
export const Clangd: Info = {
  ...
  async spawn(root) {
    ...
    await fs.symlink(bin, path.join(Global.Path.bin, "clangd")).catch(async () => {
      // Windows: symlink may require admin/developer mode, fallback to copy
      await fs.copyFile(bin, path.join(Global.Path.bin, "clangd")).catch(() => {})
    })
