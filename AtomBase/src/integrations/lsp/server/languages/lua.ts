...
export const LuaLS: Info = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      if (stats) {
        await fs.rm(installDir, { force: true, recursive: true }).catch(async (err) => {
          // Windows: retry after short delay if files are locked (EBUSY/EPERM)
          if (process.platform === "win32") {
            await new Promise((r) => setTimeout(r, 500))
            await fs.rm(installDir, { force: true, recursive: true }).catch(() => {})
          }
        })
      }
