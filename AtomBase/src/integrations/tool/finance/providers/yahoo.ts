...
async function yahooFetch(url: string): Promise<any | null> {
  ...
  try {
    ...
    if (!response.ok) {
      log.error(`Yahoo Finance HTTP ${response.status} for ${url}`)
      return null
    }
...
export async function fetchAllYahooData(
  ...
  const [price, klines] = await Promise.all([fetchYahooPrice(symbol, assetType), fetchYahooKlines(symbol, "1mo", "1d")])
