...
function detectVision(model: OllamaModel): boolean {
  ...
  const visionModels = [
    ...
    "qwen2.5-vl",
    "gemma4",
    "gemma-4", // Gemma 4 — all variants are multimodal
    "gemma3",
...
function supportsTools(model: OllamaModel): boolean {
  ...
  const toolCapableFamilies = [
    ...
    "gemma-3",
    "gemma4",
    "gemma-4", // ← Gemma 4 — full tool calling support
    "granite",
    "nemotron",
    "hermes",
    "yi",
  ...
  for (const pattern of noToolModels) {
    if (name.startsWith(pattern) || name.includes(`:${pattern}`)) {
      ...
      if (
        pattern === "phi" &&
        (name.includes("phi3") || name.includes("phi-3") || name.includes("phi4") || name.includes("phi-4"))
      ) {
        continue
      }
