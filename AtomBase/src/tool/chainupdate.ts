...
export const ChainUpdateTool = Tool.define("chainupdate", {
  ...
  parameters: z.object({
    ...
    // start action - preprocess to handle JSON string serialization
    steps: parseJsonIfString(z.union([z.array(z.string()), z.array(StepWithTodosSchema)]))
      .optional()
      .describe(
        "Step names (strings) or step objects with name, optional description, and optional todos array for 'start' action",
      ),
    // add_step action
    step_name: z.string().optional().describe("Name of the new step for 'add_step' action"),
    step_description: z.string().optional().describe("Description for 'add_step' action"),
    step_todos: parseJsonIfString(z.array(z.string())).optional().describe("Todos for 'add_step' action"),
    // update action
    status: z
      .enum([
        "pending",
    ...
    tool: z.string().optional().describe("Tool name being used for 'update' action"),
    // todo actions
    todos: parseJsonIfString(z.array(z.string())).optional().describe("Todo items for 'set_todos' action"),
    todo_index: z.number().optional().describe("Index (0-based) for 'todo_done' action"),
    // complete/fail actions
    output: z.string().optional().describe("Output message for 'complete' action"),
    error: z.string().optional().describe("Error message for 'fail' action"),
    // sub_plan action
    step_index: z.number().optional().describe("Step index (0-based) for sub_plan, sub_plan_end, and parallel_update"),
    reason: z.string().optional().describe("Reason for creating a sub-plan"),
    sub_steps: parseJsonIfString(z.array(SubStepSchema)).optional().describe("Steps for the sub-plan"),
  }),
        ...
        if (params.steps && params.steps.length > 0) {
          ...
          for (const step of params.steps) {
            if (typeof step === "string") {
              // Validate: skip if step name is too short (likely a parse error)
  ...
  formatValidationError(error: z.ZodError): string {
    ...
    for (const issue of issues) {
      ...
      if (path === "steps" && issue.code === "invalid_type") {
        return `Invalid 'steps' parameter. The steps must be an array of either:
1. Strings: ["Step 1", "Step 2"]
2. Objects with name and optional todos: [{"name": "Step 1", "todos": ["todo1", "todo2"]}]

Example: chainupdate action="start" steps=[{"name": "Setup", "todos": ["Create folder", "Install deps"]}]`
      }

      if (path.includes("action")) {
        return `Invalid action. Must be one of: start, add_step, update, complete, fail, clear, set_todos, todo_done, sub_plan, sub_plan_end, parallel_update`
      }

      messages.push(`Field '${path}': ${issue.message}`)
    }

    return `Validation failed:\n${messages.join("\n")}\n\nPlease check the chainupdate tool documentation.`
  },
})
