            ...
            case "execute": {
                ...
                while (true) {
                    ...
                    for (const task of workflow.tasks) {
                        const result = workflow.results[task.id]
                        if (result.status === "pending" && hasFailedDependency(task, workflow)) {
                    ...
                    const promises = ready.map(async (task) => {
                        ...
                        try {
                            ...
                            const model = await selectModel(task.category, fallbackModel)
                            result.model = model
                ...
                for (const task of workflow.tasks) {
                    ...
                    const duration = r.startedAt && r.completedAt
                        ...
                        : "-"
                    parts.push(`| ${task.id} | ${statusEmoji} ${r.status} | ${modelStr} | ${duration} |`)
                }
                ...
                for (const taskId of completedTasks) {
                    ...
                    if (r.output) {
                        ...
                        const truncated = r.output.length > 2000
                ...
                return {
                    ...
                    metadata: {
                        ...
                        tasks: Object.fromEntries(
                            workflow.tasks.map(t => [t.id, workflow.results[t.id].status])
                        ),
                    },
                }
            }
