...
function Invoke-SourceBuild {
    ...
    try {
        ...
        $cloneJob = Start-Job { git clone --depth 1 https://github.com/aToom13/AtomCLI.git 2>&1 }
        $chars = @('|','/','-','\'); $ci = 0
        ...
        $depsJob = Start-Job { bun install 2>&1 }
        $elapsed = 0
        ...
        $pwPkg = Start-Job { bun add playwright 2>&1 }
        $pi = 0
...
function Install-PlaywrightBrowsers {
    ...
    try {
        Write-Step "Installing Playwright package..."
        $pwJob = Start-Job { bun init -y 2>&1; bun add playwright 2>&1 }
        $chars = @('|','/','-','\'); $pi = 0
        ...
        if (Test-Path (Join-Path $playwrightDir "node_modules\playwright")) {
            ...
            $chromJob = Start-Job { bunx playwright install chromium 2>&1 }
            $ci2 = 0
