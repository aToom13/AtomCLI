...
- A final-step completion blocked by open taskflow, workflow, or child work becomes a visible `budget_exhausted/step_limit` result. It must not create a tool-less synthetic continuation or leave a reasoning-only assistant message with no terminal status.
- When the objective is complete but durable taskflow work remains unresolved, the runtime blocks new work grants (0 calls) for reconciliation and enters finalization. It does not issue redundant code/test slices when deliverables and passing verification already exist.
- Independent review steps are runtime-owned; the agent cannot mark them completed without actual reviewer invocation evidence and a matching PASS verdict.
- Watchdog tracks semantic signatures (normalizing command and working directory) and blocks repeated verification calls without new mutation evidence after three unchanged results.
- Reminder injection is tied to new mutation revisions and distinguishes production code from test, fixture, generated, and example paths to prevent false-positive loops.
- When execution limits are reached, finalization proceeds unidirectionally: work tools are disabled, the model is given one final turn and one repair attempt, and a rich deterministic fallback is generated if no usable text is returned, ensuring zero empty assistant turns.
...
- Optional `execution_budget` limits use a persistent SQLite/WAL ledger. A root request and its child sessions share atomic model-call, agent-step, duration, and cost admission; adaptive work runs in 30-call slices with append-only, user-visible, tool-free model checkpoints and bounded (50-call) slice requests. Slice extensions stop after six grants, and repeated calls, targets, errors, or consecutive no-progress slices stop new work and enter a separate tool-free final-response phase that does not consume work allowance. The report distinguishes technical, verification, and procedural harness blockers. Explicit configured ceilings remain hard. Plan revisions are retained and may reopen completed work when new evidence appears. Dispatched calls whose outcome is unknown remain conservatively charged after restart. Verification probes keep the originating execution context and abort when the caller, deadline, or execution lease is cancelled.
- Coordinated work creates a durable taskflow before normal tools run. The runtime atomically advances the first pending item to `running` before actual work starts, so evidence cannot precede the `pending → running` transition. `taskflow(action="revise")` appends new plan revisions without erasing history. External blockers wait durably for user input and resume the same execution; watchdog and extension-ceiling blockers finalize with a user-facing report.
...
- Compaction control messages continue the current execution, preserving its objective, plan, and consumed allowance; they must not cancel it or start a new root invocation.
